...
#define currency and symbol
currency = 'btc'
symbol = 'ethusd'

# unauthenticated

def ticker(symbol=symbol): # gets the innermost bid and asks and information on the most recent trade.
...
def stats(symbol=symbol): # Various statistics about the requested pairs.
...
def today(symbol=symbol): # today's low, high and volume.
...
def orderbook(symbol=symbol): # get the full order book.
...
def lendbook(currency=currency): # get the full lend book.
...
def trades(symbol=symbol): # get a list of the most recent trades for the given symbol.
...
def lends(currency=currency): # get a list of the most recent lending data for the given currency: total amount lent and rate (in % by 365 days).
...
def payloadPacker(payload): # packs and signs the payload of the request.
	...
	h = hmac.new(bytes(API_SECRET , 'latin-1'), data, hashlib.sha384)
...
def place_order(amount, price, side, ord_type, symbol=symbol, exchange='bitfinex'): # submit a new order.
...
def past_trades(timestamp=0, symbol=symbol): # view your past trades
